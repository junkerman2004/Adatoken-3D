...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            config.mm_vision_tower= "/home/Datasets/zk/clip-vit-large-patch14-336/"
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal_pdrop(
        ...
        image_idx = [idx for idx, img in enumerate(images) if img.ndim == 3]
        video_idx = [idx for idx, vid in enumerate(images) if vid.ndim == 4]
        images_minibatch = torch.stack([images[idx] for idx in image_idx]) if len(
            image_idx) > 0 else []  # mini_b_1 3 h w
        videos_minibatch = torch.stack([images[idx] for idx in video_idx]) if len(
            video_idx) > 0 else []  # mini_b_2 v 3 h w
        ...
        if getattr(videos_minibatch, 'ndim', 0) == 5:  # batch consists of videos, [mini_b_2, v, 3, h, w]
            video_features_minibatch, batch_offset = self.encode_rgbd_videos(videos_minibatch, depths, poses,
                                                                             intrinsics, lengths=lengths)
            if batch_offset is not None:  # voxelization method
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if image_token_indices == []:
                ...
                image_token_posi.append(image_token_indices[0])
            ...
            for i in range(num_specials + 1):  # num_images = 1? [0, 1]
                ...
                if i < num_specials:
                    ...
                    if special_token == IMAGE_TOKEN_INDEX:
                        ...
                        print(cur_image_features.shape[0])
                    elif special_token == LOC_TOKEN_INDEX:
                        ...
                        cur_new_input_embeds.append(cur_prompt_features)
                        cur_new_labels.append(
                            torch.full((cur_prompt_features.shape[0],), IGNORE_INDEX, device=cur_labels.device,
                                       dtype=cur_labels.dtype))
                        print(cur_prompt_features.shape[0])
                    else:
                        raise NotImplementedError
...
#####################################################################################################
        ...
        if tokenizer_model_max_length is not None:
            ...
            new_labels = [x[:tokenizer_model_max_length] for x in new_labels]
        max_len2=max(x.shape[1] for x in new_input_embeds)
