...
def adjust_causal_mask(causal_mask: torch.Tensor, target_length: int) -> torch.Tensor:
    original_length = causal_mask.shape[-1]

    if target_length == original_length:
        return causal_mask  # 长度一致，无需调整

    if target_length < original_length:
        # 裁剪
        return causal_mask[..., :target_length]

    if target_length > original_length:
        # 填充
        padding_length = target_length - original_length
        padding = torch.full(
            (1, 1, 1, padding_length),
            torch.finfo(causal_mask.dtype).min,
            device=causal_mask.device
        )
        return torch.cat([causal_mask, padding], dim=-1)
def _prepare_4d_causal_attention_mask(
    ...
    print("===2")
    print("key_value_length:", past_key_values_length)
    print(input_shape[-1] )
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.image_token_ratio_list=[1,0.5,0.25,0.125]
            ...
            if rank_layer in self.layer_list:
                if hidden_states.shape[1] != 1:  # prefill stage or training
                    ...
                    if self._use_flash_attention_2:
                        ...
                        attention_mask = _prepare_4d_causal_attention_mask(
                            attention_mask, (batch_size, new_seq_length), inputs_embeds, past_key_values_length
                        )
                    ...
                    cur_visual_length = [int(cur_image_token * self.image_token_ratio_list[stage]) for cur_image_token
                                         in self.image_tokens]
                    next_visual_length = [int(cur_image_token * self.image_token_ratio_list[stage + 1]) for
                                          cur_image_token in self.image_tokens]
                    if stage==0:
                        mm=past_key_values_length+seq_length-cur_visual_length[0]+next_visual_length[0]
                    else:
                        mm-=cur_visual_length[0]-next_visual_length[0]

                    attention_mask = adjust_causal_mask(attention_mask, mm)
                    new_position_ids = []
    ...
    def pdrop_rank_drop(
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":
            ...
            print(self.image_token_ratio_list)
            print(self.layer_list)
            image_tokens = [int(cur_image_token * self.image_token_ratio_list[cur_num]) for cur_image_token in
                            self.image_tokens]
            keep_length = [int(cur_image_token * self.image_token_ratio_list[cur_num + 1]) for cur_image_token in
                           self.image_tokens]
            print("image tokens",int(image_tokens[0]))
            print("keep length",int(keep_length[0]))
            features_list = []
            attention_mask_list = []
            labels_list = []
            ...
            if attention_mask is not None:
                if attention_mask.dim() == 4:
                    valid_mask = torch.ones((batch_size, features.shape[1]), dtype=torch.bool, device=features.device)
                else:
                    valid_mask = attention_mask.bool()
            else:
                valid_mask = torch.ones_like(features[:, :, 0]).bool()  # [B, S
            print(features.shape)
            print(valid_mask.shape)
            print(labels.shape)
            features = [f[m] for f, m in zip(features, valid_mask)]
            labels = [l[m] for l, m in zip(labels, valid_mask)]
            attention_mask = [m[m] for m in valid_mask]
            ...
            for i in range(batch_size):
                ...
                attn_weights = torch.matmul(text_query_states, cur_key_states.transpose(1, 2)) / math.sqrt(
                    head_dim)  # (num_head, text_token,seq_len)
                attn_weights = attn_weights + text_eager_attention_mask
                attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(
                    query_states.dtype)  # (num_head, text_token,seq_len)

                attention_avg_head = torch.mean(attn_weights, dim=0)  # ave across heads
                attention_avg_head = attention_avg_head[:,
                                     image_index:image_index + image_tokens[i]]  # select image token as keys
                ...
                new_labels = torch.cat([labels[i][:image_index], labels[i][top_rank_index], labels[i][start_index:]],
                                       dim=0)

                new_attention_mask = torch.cat([attention_mask[i][:image_index], attention_mask[i][top_rank_index],
                                                attention_mask[i][start_index:]], dim=0)
            ...
            for i, (cur_new_embed, cur_new_labels) in enumerate(zip(new_input_embeds, new_labels)):
                ...
                new_seq_len = torch.sum(cur_attention_mask).item()
                m=torch.full((dif,), False, dtype=cur_attention_mask.dtype,
            ...
            return position_ids, new_attention_mask, new_input_embeds, new_labels,new_seq_len
