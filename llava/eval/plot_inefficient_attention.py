...
def visualize_attention(multihead_attention,output_path="atten_map_1.png",title="Layer 5"):
    ...
    print("a",averaged_attention.shape[0],"b:", averaged_attention.shape[1])
...
if __name__=="__main__":
    ...
    parser.add_argument("--layer_list", type=str, default="[10, 20, 25]")
    parser.add_argument("--image_token_ratio_list", type=str, default="[0.8,0.6,0.4,1]")
    pargs = parser.parse_args()

        # %%
    class InferenceArgs:
        model_path = pargs.model_path
        layer_list=pargs.layer_list
        image_token_ratio_list=pargs.image_token_ratio_list
        model_base = None
        image_file = None
        device = "cuda"
        conv_mode = None
    ...
    def inference(prompts,images,append_output=""):
        ...
        for prompt,image in tqdm(zip(prompts,images),total=len(prompts)):
            ...
            image_tensor = process_images([image], image_processor['image'], args)
            print(image_tensor)
